...
## CADENCE AMENDMENT — TUESDAY BASELINE  [2026-08-18, operator-authorized]
Root cause audit: the weekly systems' FRED daily inputs (H.15 yields DGS2/10/30, T10Y2Y, T10YIE; Moody's BAA10Y; VIXCLS; DFF) publish the prior Friday's values Monday afternoon-evening. A Monday-morning print is therefore impossible (freshness guard abstains correctly); Wednesday 01:15 was one day later than necessary. Thursday claims covering the issue week publish AFTER any run and never enter a print (asof-backward) — no later run gains data.
CHANGES (both systems):
* AEGIS: NEW com.zoltar.weekly.tue (Tue 01:15) = the BASELINE weekly print. com.zoltar.weekly.wed retained as PURE CATCH-UP — ZOLTAR_WEEKLY_GUARDED.sh now also skips when a fresh covered ticket (<26h, any sleeve non-NO_SIGNAL) exists. Monday jobs com.zoltar.weekly (Mon 01:00) and com.zoltar.aegis (Mon 06:00) REMOVED (unloaded; plists archived in ~/Desktop/ZOLTAR_BACKUPS_20260818_tuesday_baseline/ with pre-patch guard + plists).
* ZION: NEW com.zoltar.zion.universe.tue (Tue 01:15) runs the same run_universe.sh as an EXECUTION-BASELINE refresh: panel rebuilt with complete macro, desk ticket regenerated (the sheet the operator trades), reconcile re-run. The Friday 17:30 job is UNCHANGED and remains the as-issued tape moment (issuing Tuesday would let 1.5 days of the scored week elapse pre-issue — tape stays clean). Tape rows are never duplicated or rewritten by the Tuesday run (same-week guard).
* Desk ticket TIMING: ENTER Tuesday at/near the open from the Tuesday sheet; ROLL at next Tuesday print. Paper mon-close tracker already treats Tue-Thu prints as the traded mid-week print — cadence-compatible.

## CANDIDATE C1 — silver micro resize 0.05 -> 0.075  [2026-08-18, FORWARD CANDIDATE, NOT LIVE]
Re-tested silver micro (VIX/IP_Nowcast N8/H4 episodic; already LIVE at W_MICRO=0.05) against the CURRENT book WITH UUP+India, rebuilt from netting_ledger exposures x realized leg returns (leverage-invariant, so Sortino/corr unaffected by the leverage layer).
RESULT: silver micro HELPS (G4 PASS; +5% lifts book Sortino 1.563 -> 1.600, dSort +0.037) but misses G3 decorrelation (corr +0.32, down from +0.39 pre-UUP/India — UUP dollar-long vs silver dollar-short pulled it toward the 0.30 gate). Earns its place via positive risk-adj return, NOT diversification.
SWEEP (current book): Sortino peaks ~10% (1.617) vs live 5% (1.600); -10.4% DD budget holds to ~20%, breaks at 30% (-13.4%). Doubling 5->10% buys only +0.017 Sortino (noise-level). 7.5% = sweet spot (most of the gain, concentration-safe, DD-neutral).
CANDIDATE: W_MICRO 0.05 -> 0.075.
BAR (pre-registered, FORWARD): over 12+ forward weeks with the silver micro ACTIVE, the 7.5% weighting must not reduce book Sortino vs 5% and must hold the -10.4% DD budget. Scored from the as-issued tape's SILVER leg (return x weight delta), NOT backfill.
STATUS: NOT LIVE — W_MICRO stays 0.05. Live change needs operator authorization AFTER the bar is met. Caveats: silver is thin (77 active wks), gold-beta, 2025-concentrated, mostly backfill — do not over-weight a thin correlated sleeve.

## LEVERAGE RULE — declared 2026-08-18 (operator)
L_max = 0.90 × Sortino (raised from 0.80, operator 2026-08-18), where:
  * Sortino = the FORWARD (or haircut) universe Sortino, NOT raw backtest. Backtest metrics inflate OOS (house doctrine) — use the as-issued forward Sortino as it accrues, or haircut ~20-30% before applying.
  * Sortino is leverage-INVARIANT (1x == 4x), so it is a stable input; plug in the unleveraged universe Sortino.
  * Applies to PEAK leverage. ZION leverage is STATE-DEPENDENT — throttle flattens sleeves in 41% of weeks (stress), the 2.0x recovery boost fires only 9% of weeks, mean structural gross is 0.70x. So peak-Nx is materially safer than constant-Nx; the rule is conservative applied to a throttled book.
DD BACKSTOP = 20% (REVISED UP from 15.7%, operator 2026-08-18): hard circuit breaker independent of the Sortino rule. Take the TIGHTER of {0.9 x Sortino, the leverage that hits 20% DD}.
RECONCILE FIRST: universe_book.py (live pipeline) reports Sortino 4.13 -> 0.9x = 3.7x; Amdt-5 / 3.8x-model backtests report 5.20-5.96 -> 4.7-5.4x. Required Sortino to justify 4x at 0.9x = 4.44. The 4.13-vs-5.x gap IS the difference between permitting 3.7x and 5.4x — pin down the true universe Sortino (prefer forward) before levering to 4x.
STATUS: destination framework. Live ladder UNCHANGED (2.5x now -> 3.2 -> 4.0 on forward gates). Raising live UNIVERSE_LEV or the DD budget in executing code is a SEPARATE step needing explicit authorization — NOT done here.

## SORTINO RECONCILIATION — 2026-08-18 (forward tape)
The backtest universe Sortino is NOT a single robust number: 2.79 (independent reconstruction, current +UUP/India book) → 4.14 (pipeline universe_book.py, old locked-book) → 5.20-5.96 (Amdt-5 / 3.8x-model). 2x spread from reconstruction + config + weekly:monthly blend-leverage. Forward tape has 1 resolved week — a Sortino needs >=12, so forward cannot arbitrate yet.
RESOLUTION: hold the CONSERVATIVE end (2.8) for the leverage rule until forward matures. 0.9 x 2.8 = 2.52x — which is EXACTLY the live 2.5x ladder. 4x requires a FORWARD Sortino >= 4.44 (~12+ weeks). Tracker forward_sortino.py (run_universe.sh step 6/8) recomputes each week and logs reports/forward_sortino_log.csv. Do NOT lever toward 4x on the optimistic backtest end — earn it on the forward tape.

## SORTINO RECONCILIATION — CORRECTION 2026-08-18
Traced the optimistic Sortino to source: reports/universe_monthly_backtest.csv (the CHARTED backtest) = Sortino 5.20, CAGR 47%, MaxDD -15.7% @4x; leverage-invariant (5.20 at 1x/2.54x/4x — verified). This is the fully-amended (Amdt 1-6) book. The earlier "2.79 independent reconstruction" was a DATA-PROXY ARTIFACT (SI=F/GC=F + own fetches; it fell BELOW the pipeline's own pre-amendment 4.14, so it understated — discard it, not a hidden-risk find). "5.96" not in files -> superseded by 5.20.
NET: trustworthy backtest ~5.20. Haircut 25% -> 3.9 -> 0.9x = 3.5x supportable now; charted 5.20 -> 0.9x = 4.68x. 4x is a LEGITIMATE DESTINATION (needs forward Sortino >= 4.44, plausible given 5.20), NOT a stretch. Live ladder held at 2.5x by forward-WEEK gates, not the Sortino. forward_sortino.py anchors updated: BT_CONSERVATIVE=3.9, BT_CHARTED=5.20.

## CANDIDATE C2 — EQUITY DE-CONCENTRATION (inverse-vol SPY/QQQ split)  [2026-08-18, FORWARD, NOT LIVE]
Origin: operator's correlation-hedge idea. The full mean-variance / target-return version was REJECTED (error-maximization: a min-var solve dumps 76% into SHY and zeroes equities; return-targeting needs an unforecastable mu; fitted correlations break in stress). This is its DEFENSIBLE KERNEL — a structural, no-forecast rule.
RULE: split the equity risk-block (US_EQ+NASDAQ) between SPY and QQQ by INVERSE 26-week volatility instead of by Sortino. Total equity weight and ALL structural hedges (2Y/gold/dollar/India/silver) UNCHANGED. No correlation estimation, no return target, no optimizer, no corner solutions — only a stable descriptive statistic (vol).
WHY: SPY/QQQ are ~0.94 correlated (one bet), yet Sortino-weighting silently lets higher-vol QQQ carry ~67% of book risk (SPY 28%). Inverse-vol balances the two. Latest split flips 0.39/0.61 -> 0.60/0.40 (QQQ vol 3.1%/wk vs SPY 2.0%).
BACKFILL BACKTEST: book Sortino 1.598 -> 1.639 (+0.041), MaxDD -10.4% -> -10.2% (DD-neutral). Modest, honest, structural.
FORWARD SHADOW: paper_deconcentration.py (run_universe.sh step 5/9) recomputes actual vs inverse-vol book each resolved week; log reports/paper_deconcentration_log.csv. Bar: 12+ forward weeks, de-conc Sortino >= actual, DD not worse. NOT LIVE — equity split stays Sortino-weighted; live change needs operator auth after the bar.
NOTE: this honors the correlation instinct WITHOUT importing the fragility — it de-concentrates by risk, it does not forecast or target a return.
