...
echo "--- [4/9] paper Mon-close rule ---";        (cd "$U"  && $PY paper_monclose.py)        || echo "PAPER MON-CLOSE FAILED"
echo "--- [5/9] paper de-concentration (C2) ---"; (cd "$U"  && $PY paper_deconcentration.py) || echo "PAPER DE-CONC FAILED"
echo "--- [6/9] ZION vs LIVE comparison ---";     (cd "$U"  && $PY zion_vs_live.py)          || echo "COMPARISON FAILED"
echo "--- [7/9] forward Sortino reconcile ---";   (cd "$U"  && $PY forward_sortino.py)       || echo "FORWARD SORTINO FAILED"
echo "--- [8/9] ZION desk ticket ---";            (cd "$U"  && $PY zion_desk_ticket.py)      || echo "TICKET FAILED"
echo "--- [9/9] fills reconcile (read-only) ---"; (cd "$U"  && $PY zion_reconcile.py)        || echo "RECONCILE SKIPPED (token/creds — see output)"
echo "=== done $TS ==="
